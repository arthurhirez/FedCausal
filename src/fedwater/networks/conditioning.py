...
Ported from the ``network_assessment`` notebook, model-first: every entry point
takes a loaded ``wntr`` model plus a name, so a Kedro node can hand it the
catalog's ``network_inp`` dataset instead of a path. ``from_file`` and the
``*_corpus`` helpers keep the path-based notebook workflow.

Option pinning is delegated to :mod:`fedwater.networks.options`, which the
simulation's ``network_prep`` also uses -- otherwise the assessment could
certify a model the simulation never runs.
...
class ConditioningConfig:
    ...
    inpfile_units: str = "LPS"                # units of any .inp written out
    expected_headloss: str = "H-W"            # asserted, never coerced; "" skips
...
def fold_demand_multiplier(wn, recipe: Recipe):
    ...
    NOTE the opposite convention on the simulation side: ``network_prep`` PINS
    the multiplier without folding, because ``build_portfolios`` reads the raw
    ``.inp`` base demands as its demand anchor. Folding there would rescale the
    whole scenario by the file's multiplier (5x on Graeme, which ships 0.2).
    """
    r = opt.fold_demand_multiplier(wn)
    if not r["folded"]:
        recipe.log("fold_demand_multiplier", False,
                   "multiplier already 1.0", multiplier=r["multiplier"])
        return wn
    recipe.log("fold_demand_multiplier", True,
               f"folded {r['multiplier']} into {r['n_entries']} demand entries; "
               f"option pinned to 1.0",
               multiplier=r["multiplier"], n_entries=r["n_entries"])
    return wn
...
def normalize_pattern_lengths(wn, recipe: Recipe, target: int):
    ...
    """
    fixed = opt.normalize_pattern_lengths(wn, target)
...
def set_time_and_solver_options(wn, recipe: Recipe, cfg: ConditioningConfig):
    ...
    if cfg.duration_h is not None:
        opt.pin_time(wn, cfg.duration_h, cfg.timestep_s)
    opt.pin_demand_model(wn, cfg.demand_model, cfg.required_pressure_m,
                         cfg.minimum_pressure_m)
    opt.pin_inpfile_units(wn, cfg.inpfile_units)
...
def condition_network(wn, name: str, cfg: ConditioningConfig | None = None,
                      source: str | Path = "") -> tuple[object, Recipe]:
    """Apply the full conditioning chain to a model. Returns (model, recipe).

    The input model is deep-copied, never mutated: the caller's ``wn`` (a Kedro
    catalog dataset, shared across nodes) must come back untouched.
    ...
    wn = copy.deepcopy(wn)
    recipe = Recipe(name, source or name, cfg)

    if cfg.expected_headloss:
        recipe.log("check_headloss", False,
                   f"headloss={opt.check_headloss(wn, cfg.expected_headloss)}",
                   expected=cfg.expected_headloss)
...
def from_file(source: str | Path):
    """Load an ``.inp`` as (model, name). Convenience for notebook use."""
    source = Path(source)
    return wntr.network.WaterNetworkModel(str(source)), source.stem
...
def auto_condition(wn, name: str, cfg: ConditioningConfig | None = None,
                   ladder: tuple[str, ...] = DEFAULT_POLICY_LADDER,
                   pressure_floor_m: float = 15.0,
                   source: str | Path = "") -> tuple[object, Recipe, pd.DataFrame]:
    ...
    for policy in ladder:
        ...
        wn, recipe = condition_network(base, name, trial_cfg, source)
        if not consumer_junctions(wn):
            attempts.append({"network": name, "policy": policy,
    ...
    if chosen is None:
        ...
        wn, recipe = condition_network(
            base, name, ConditioningConfig(**{**asdict(cfg),
                                              "storage_policy": ladder[0]}), source)
...
def condition_corpus(paths, cfg: ConditioningConfig | None = None,
    ...
    for p in paths:
        ...
        t0 = time.time()
        try:
            base, name = from_file(p)
            wn, recipe, log = auto_condition(base, name, cfg, ladder,
                                             pressure_floor_m, source=p)
