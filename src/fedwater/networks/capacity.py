...
Ported from the ``network_assessment`` notebook, model-first: ``assess_capacity``
takes a loaded ``wntr`` model plus a name so a Kedro node can hand it the
catalog's ``network_inp`` dataset. ``assess_corpus`` keeps the path-based
notebook workflow.
...
from . import conditioning as nc
...
def _prepare(wn, name: str, cfg: CapacityConfig, tank_level: str, mode: str):
    ...
    ccfg = nc.ConditioningConfig(
        ...
        expected_headloss=cfg.expected_headloss,
    )
    out, recipe = nc.condition_network(wn, name, ccfg)
    if not cfg.freeze_tanks and tank_level != "initial":
        # hold tanks at the chosen level without changing their type
        for t in out.tank_name_list:
            tank = out.get_node(t)
...
def assess_capacity(wn, name: str,
    ...
    ``wn`` is never mutated -- every solve runs on a conditioned deep copy.
    """
    cfg = cfg or CapacityConfig()
    net = name

    # Static analysis MUST see the same demand convention the sweep uses.
    # `_prepare` folds the file's global multiplier into the base demands so
    # that lambda is the only knob; reading base demands off the unfolded model
    # here would report `base_demand_lps` on one scale and scale it by a lambda
    # measured on another. On a file shipping multiplier 0.2 that is a silent
    # 5x error in `max_demand_lps` -- the number the anchor is derived from.
    folded = copy.deepcopy(wn)
    multiplier = opt.fold_demand_multiplier(folded)["multiplier"]
    static = static_head_analysis(folded)
    ...
    probe_wn, _ = _prepare(wn, net, cfg, cfg.tank_levels[0], "DD")
    ...
    for tank_level in cfg.tank_levels:
        for mode in cfg.modes:
            model, _ = _prepare(wn, net, cfg, tank_level, mode)
            if not nc.consumer_junctions(model):
                summaries.append({"network": net, "tank_level": tank_level,
                                  "mode": mode, "status": "no demand in file",
                                  "lambda_star": np.nan})
                continue

            grid = [_evaluate(model, lam, excluded, cfg)
            ...
            row = {"network": net, "tank_level": tank_level, "mode": mode,
                   ...
                   # FOLDED convention (see above). The SIMULATION pins the
                   # multiplier instead and anchors on the RAW base demands, so
                   # `base_demand_lps_raw` in the inventory is a different
                   # number whenever the file ships a multiplier != 1.
                   "demand_convention": "folded",
                   "inp_demand_multiplier": multiplier,
            ...
            if feas.empty:
                ...
                lam_star, n = bisect_lambda(model, excluded, cfg, lo, hi)
            ...
            if not np.isnan(row["lambda_star"]):
                ...
                if mode == "DD" and tank_level == cfg.tank_levels[0]:
                    b = binding_diagnosis(model, row["lambda_star"], static,
                                          excluded, cfg)
                    if not b.empty:
...
def assess_corpus(paths, cfg: CapacityConfig | None = None,
    ...
    for p in paths:
        t0 = time.time()
        try:
            base, name = nc.from_file(p)
            out = assess_capacity(base, name, cfg)
