"""browsers.py -- the three interactive placement composites.

Wiring only: every number shown comes from a table `placement.store` persisted
...
see is what the store holds and they work on a `placement.store.load_stack`ed result
...
from . import classification as pl
...
def _require():
    try:
        ...
        raise ImportError("the browsers need ipywidgets, IPython and "
                          "matplotlib; `placement.figures` works without "
                          "them") from exc
