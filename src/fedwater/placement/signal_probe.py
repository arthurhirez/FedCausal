...
__all__ = ["Probe", "from_world", "weekly_profile",
           "aligned_profile", "district_profile",
...
class Probe:
    ...
    # Memo store, keyed by (what, window, candidate-order). NOT part of the
    # world's identity and never persisted: it holds derived arrays only, so
    # a cold cache and a warm one return bit-identical results. See
    # `aligned_profile` for why this exists.
    ...
    def from_world(cls, world: dict) -> "Probe":
        """From one probe world (``placement.excite.simulate_probe`` output,
        or ``placement.store.load_world``).

        ``world`` carries ``params`` (the RESOLVED blocks the world was run
        with), ``districts`` (the partition document), the three series, the
        drift schedule, the wntr model the world was solved on, and a label.
        """
        p = world["params"]
        dy = world["districts"]
...
def candidates(P: Probe, kinds=("pressure", "flow"), subtree: bool = True,
    ...
    """
    key = ("cand", tuple(kinds), bool(subtree), bool(consumers_only))
    hit = P._cache.get(key)
    if hit is not None:
        # a copy, because every caller adds columns to what it gets back
        return hit.copy()
    ...
    df = df.sort_values(["kind", "district", "id"]).reset_index(drop=True)
    P._cache[key] = df
    return df.copy()
...
def aligned_profile(P: Probe, cand: pd.DataFrame, lo: int, hi: int):
    """(sensor weekly profile, alignment sign) over a step window, MEMOISED.

    The alignment and the weekly fold are exactly what `profiles` and
    `mixture_probe._profile` were each doing inline; they now share one
    implementation and one cache. That is a cost change, not a result change:
    `weekly_profile` is a deterministic function of the raw slice, the
    candidate ORDER and `steps_day`, all three of which are in the key.

    The cost it removes is not marginal. One `world_response` call profiled
    the same three windows ten times over ~840 candidates -- twice for the
    measurement, four times for the shape/native split-half deltas, four more
    for the matched and raw nulls. At one world that is invisible; at twenty
    it is most of the runtime.
    """
    # the id tuple itself is the key: no builtin hash(), which is salted
    # per process (a memo key only, but the project keeps hash() out entirely)
    key = ("prof", int(lo), int(hi), tuple(cand["id"]))
    hit = P._cache.get(key)
    if hit is None:
        ...
        S = weekly_profile((raw * sign[:, None]).T, P.steps_day, step0=lo)
        hit = (S, sign)
        P._cache[key] = hit
    S, sign = hit
    return S.copy(), sign.copy()


def district_profile(P: Probe, lo: int, hi: int) -> np.ndarray:
    """District demand weekly profile over a step window, MEMOISED."""
    key = ("dem", int(lo), int(hi))
    hit = P._cache.get(key)
    if hit is None:
        dd = P.district_demand.iloc[lo:hi].to_numpy(float)
        hit = np.atleast_2d(weekly_profile(dd, P.steps_day, step0=lo))
        P._cache[key] = hit
    return hit.copy()
...
def profiles(P: Probe, cand: pd.DataFrame | None, phase: str) -> dict:
    ...
    demand = district_profile(P, lo, hi)
    ...
    sensor, sign = aligned_profile(P, cand, lo, hi)
...
def canonical_profile(P: Probe, income: str, land_use: str,
    ...
    """
    from fedwater.pipelines.demand_synthesis import nodes as DS
    from fedwater.pipelines.urban_scenario import nodes as US
