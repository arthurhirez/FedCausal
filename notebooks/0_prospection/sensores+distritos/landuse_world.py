...
def summary(world: dict) -> pd.Series:
    ...
    # `tgt` is None when neither the cfg nor the profile named a drift target.
    # Indexing the partition with None raised a KeyError from inside a summary
    # line, which is a confusing place to lose a world that solved fine.
    tgt_nodes = set(world["districts"]["districts"].get(tgt, []))
