...
def _w_cols(table) -> list[str]:
    return [c for c in table.columns if c.startswith("w_District")]
...
def emit(res, out_root, arms=PIPELINE_ARMS, n_max: int = 5,
         extra_arms=("unusable",), enforce_channel: bool = True,
         min_distance: float = DEDUPE_MIN_DISTANCE,
         verbose: bool = True) -> pathlib.Path:
    """Write the arms, the classification, the channel verdict and the handoff.
    ...
    Layout, under `<out_root>/<config_id>/`:
        ...
        channels.csv                     per kind: does it carry a placement?
    ...
    ch = channel_diversity(table)
    ch.insert(0, "config_id", res.config_id)
    ch.to_csv(out / "channels.csv", index=False)
    dead = sorted(ch.loc[ch["verdict"] != "ok", "kind"])

    all_arms = tuple(arms) + tuple(a for a in extra_arms if a not in arms)
    for arm in all_arms:
        sel = select(table, res.names, arm, n_max=n_max, channels=ch,
                     enforce_channel=enforce_channel, min_distance=min_distance)
        note = (f"# channels dropped as degenerate: {', '.join(dead)}\n"
                if dead and arm != "unusable" else "")
        (pl / f"{arm}.yml").write_text(
            f"# {res.config_id} -- arm {arm}: {ARMS[arm].doc}\n"
            f"# <= {n_max} DISTINCT readings per district per kind "
            f"(min mixture distance {min_distance}); short sets ship RAGGED.\n"
            + note + sensors_yaml(sel) + "\n")

    sel_tab = selection_table(table, res.names, res.config_id, arms=all_arms,
                              n_max=n_max, channels=ch,
                              enforce_channel=enforce_channel,
                              min_distance=min_distance)
    cov = coverage(table, res.names, res.config_id, arms=all_arms, n_max=n_max,
                   channels=ch, enforce_channel=enforce_channel,
                   min_distance=min_distance)
    if len(sel_tab):
    ...
    if verbose:
        for _, r in ch.iterrows():
            print(f"  channel {r['kind']:8s} {r['verdict']:10s} "
                  f"tv {r['mean_pairwise_tv']:.3f} | "
                  f"{r['distinct_rows']}/{r['n_live']} distinct "
                  f"({r['distinct_share']:.1%}) | eff rank {r['eff_rank']:.2f}")
        for arm in all_arms:
            c = cov[cov["arm"] == arm]
            red = c["redundancy"].mean()
            tail = f" | redundancy {red:.1f}x" if np.isfinite(red) else ""
            print(f"  {arm:13s} {int(c['n_selected'].sum()):4d} gauges | "
                  f"{int((~c['complete']).sum())}/{len(c)} sets short | "
                  f"{int(c['empty'].sum())} empty{tail}")
    return out
