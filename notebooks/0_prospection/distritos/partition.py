...
def _community(g: nx.Graph, k: int, method: str) -> pd.Series:
    ...
    Vertex order is fixed to ``sorted(g.nodes())`` so ties break identically
    across runs and processes: igraph's community routines are deterministic
    given a vertex order, but the order of ``nx.Graph.nodes()`` follows
    insertion, which makes the result depend on link order in the ``.inp``.
    """
    import igraph as ig

    nodes = sorted(g.nodes())
    idx = {n: i for i, n in enumerate(nodes)}
    edges = sorted((min(idx[a], idx[b]), max(idx[a], idx[b])) for a, b in g.edges())
