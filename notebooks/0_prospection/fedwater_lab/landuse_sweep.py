...
__all__ = ["SweepCfg", "run_sweep", "beta_ladder", "leak_matrix",
           "signal_carriers", "drift_carriers", "stability",
           "plot_beta_ladder", "plot_leak", "plot_carriers"]
...
class SweepCfg:
    ...
    def base(self, network: str) -> lw.WorldCfg:
        return lw.WorldCfg(
            network=network, to_income=self.to_income,
...
def _cell_probe(wc: lw.WorldCfg, root, out_root, cache: bool):
    ...
    if cache and (d / "manifest.json").exists():
        from fedwater_lab import load_world
...
# read-outs
# ==========================================================================
def beta_ladder(C: pd.DataFrame) -> pd.DataFrame:
    """The strength ladder, per network and drifting district.

    Read it bottom-up: the SMALLEST beta at which `resp_flow_top` is already
    high is the answer to "how much level change does detection need". If that
    is beta 0, the shape signature alone carries the drift and the attribution
    protocol is fine. If it is 0.6 or 1.0, the detector is reading a level
    change and the protocol needs work.
    """
    cols = ["d_demand_%", "shape_shift", "d_pressure", "gap_flow",
            "gap_pressure", "resp_flow_top", "resp_pressure_top",
            "resp_leak_p95", "status"]
    d = C[["network", "drift_district", "beta"] + [c for c in cols if c in C]]
    return d.sort_values(["network", "drift_district", "beta"]).reset_index(drop=True)


def beta_threshold(C: pd.DataFrame, metric: str = "resp_flow_top",
                   threshold: float = 0.5) -> pd.DataFrame:
    """Smallest beta clearing `threshold`, per network and district.

    NaN means no beta in the grid cleared it -- which is the interesting
    outcome, not a missing value.
    """
    ok = C[(C["status"] == "ok") & (C[metric] >= threshold)]
    out = (ok.groupby(["network", "drift_district"])["beta"].min()
           .rename(f"min_beta[{metric}>={threshold}]"))
    full = (C[C["status"] == "ok"].groupby(["network", "drift_district"])
            .size().rename("cells_ok"))
    return pd.concat([full, out], axis=1).reset_index()
...
# figures
# ==========================================================================
def plot_beta_ladder(C: pd.DataFrame, metric: str = "resp_flow_top", axes=None):
    """`metric` against beta, one panel per network, one line per district."""
    import matplotlib.pyplot as plt
    nets = sorted(C["network"].unique())
    if axes is None:
        _, axes = plt.subplots(1, len(nets), figsize=(5.6 * len(nets), 3.6),
                               squeeze=False)
        axes = axes[0]
    for ax, net in zip(np.atleast_1d(axes), nets):
        d = C[(C["network"] == net) & (C["status"] == "ok")]
        for district, g in d.groupby("drift_district"):
            g = g.sort_values("beta")
            ax.plot(g["beta"], g[metric], marker="o", ms=4, label=district)
        fail = C[(C["network"] == net) & (C["status"] != "ok")]
        for _, r in fail.iterrows():
            ax.axvline(r["beta"], color="crimson", ls=":", lw=.8, alpha=.5)
        ax.set(xlabel="beta (drift strength)", ylabel=metric, title=net)
