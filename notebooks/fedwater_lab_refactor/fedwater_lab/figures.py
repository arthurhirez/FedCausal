...
# --------------------------------------------------------------------------
# prototype reconstruction (see `recon`)
# --------------------------------------------------------------------------
# One style per source, stable across figures and the browsers.
SOURCE_STYLE = {
    "true": dict(color="black", lw=2.2, label="sensors (true mean window)"),
    "local": dict(color="#DD8452", lw=1.4, label="decoded local"),
    "post_fedavg": dict(color="#4C72B0", lw=1.4, label="decoded post-FedAvg"),
    "post_fedavg_full": dict(color="#4C72B0", lw=1.4, ls="--",
                             label="decoded post-FedAvg (all windows)"),
    "finch": dict(color="#55A868", lw=1.4, label="decoded FINCH cluster"),
    "own": dict(color="#8C8C8C", lw=1.1, ls=":", label="own district demand"),
    "mix": dict(color="#C44E52", lw=1.1, ls=":", label="mixture demand"),
}


def _on_scale(x, ref, sign):
    """Show a demand shape on the channel's scale: mean(ref) +
    sign * std(ref) * z(x). Display only -- metrics use z-scores directly."""
    x = np.asarray(x, float)
    s = x.std()
    z = (x - x.mean()) / s if s > 1e-12 else np.zeros_like(x)
    return ref.mean() + sign * ref.std() * z


def plot_reconstruction(ax, arrays: dict, channel: int, own=None, mix=None,
                        title: str = ""):
    """One channel of one (client, month-or-period): the true mean window,
    every decoded source, and -- if given -- the own-district and mixture
    demand shapes drawn on the true window's scale (sign matched to the
    true window's correlation with each).

    `arrays` maps source name -> (W, F) array (`"true"` included when
    available).
    """
    ref = arrays.get("true")
    for name, a in arrays.items():
        if a is None:
            continue
        ax.plot(a[:, channel], **SOURCE_STYLE.get(name, dict(label=name)))
    if ref is not None:
        r = ref[:, channel]
        for name, x in (("own", own), ("mix", mix)):
            if x is None:
                continue
            c = np.corrcoef(r, x)[0, 1] if r.std() > 0 and np.std(x) > 0 else 1
            ax.plot(_on_scale(x, r, np.sign(c) or 1), **SOURCE_STYLE[name])
...
def plot_finch_month(ax, centroids: dict, true: dict, membership, month: int,
                     channel: int):
    """FINCH at one month: each decoded cluster centroid (thick) and the true
    mean windows of the clients assigned to it (thin, same colour).

    `centroids` = {cluster: (W, F)}; `membership` = `recon.finch_membership`.
    """
    mem = membership[membership["month"] == month]
    for j, (cl, a) in enumerate(sorted(centroids.items())):
        col = PALETTE[j % len(PALETTE)]
        members = mem.loc[mem["cluster"] == cl, "client"].tolist()
        ax.plot(a[:, channel], color=col, lw=2.4,
                label=f"cluster {cl}: {', '.join(members) or '(no member)'}")
        for c in members:
            t = true.get((c, month))
            if t is not None:
                ax.plot(t[:, channel], color=col, lw=0.9, alpha=0.6)
    ax.set(xlabel="step (window resolution)", ylabel="scaled value")
    ax.set_title(f"FINCH centroids, month {month} (thin: member clients' "
                 "true mean windows)", fontsize=9, loc="left")
    ax.legend(fontsize=7)
    return ax


def plot_retrieval_summary(ax, summary: pd.DataFrame, metric: str = "gain",
                           source: str = "true"):
    """Mean `metric` per phase, one bar group per (kind, slot_class)."""
    d = summary[summary["source"] == source]
    groups = d[["kind", "slot_class"]].drop_duplicates().values.tolist()
    phases = [p for p in ("init", "transition", "final")
              if p in set(d["phase"].astype(str))]
    width = 0.8 / max(len(groups), 1)
    for g, (kind, cls) in enumerate(groups):
        sub = d[(d["kind"] == kind) & (d["slot_class"] == cls)]
        sub = sub.set_index(sub["phase"].astype(str))
        vals = [sub[metric].get(p, np.nan) for p in phases]
        ax.bar(np.arange(len(phases)) + g * width, vals, width,
               label=f"{kind}/{cls}", color=PALETTE[g % len(PALETTE)])
    ax.axhline(0, color="k", lw=0.8)
    ax.set_xticks(np.arange(len(phases)) + width * (len(groups) - 1) / 2)
    ax.set_xticklabels(phases)
    ax.set(ylabel=metric, title=f"{metric} by phase ({source})")
    ax.legend(fontsize=7)
    return ax
