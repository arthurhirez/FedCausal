...
_INDEX_COLS = ["world_hash", "run_key", "label", "tag", "network",
               ...
               "n_include", "n_exclude", "transform", "scaling",
               "interval_agg_h", "window_size", "step_size",
               "reference_months", "lstm_units", "rounds", "fl_seed",
               "sensor_hash", "n_windows", "seconds", "saved_at", "status"]
...
def _spec_from_dict(d: dict) -> CellSpec:
    ...
    return CellSpec(
        kinds=tuple(d["kinds"]), classes=tuple(d.get("classes") or CLASSES),
        ...
        # is what a pre-fix save actually ran with (snapshot_rounds="sparse",
        # snapshot_points=1500, keep_weights="final" were the only values in
        # use, since the field couldn't be recorded any other way before).
        snapshot_rounds=(tuple(d["snapshot_rounds"])
...
class Store:
    ...
    def key_for(self, world: World, spec: CellSpec) -> tuple[str, str]:
        """(run_key, sensor_hash) -- resolves the selection to get the hash."""
        from .data import resolve_placement, sensor_hash
        sh = sensor_hash(resolve_placement(world, spec), spec.kinds)
    ...
    def save_run(self, world: World, spec: CellSpec, res: dict) -> pathlib.Path:
        ...
        (out / "sensors.json").write_text(json.dumps(
            {"sensor_hash": sh, "placement_source": world.placement_source,
             "kinds": list(spec.kinds), "classes": list(spec.classes),
             "include": list(spec.include), "exclude": list(spec.exclude),
        ...
        meta = {"run_key": key, "world_hash": world.sim_hash,
                ...
                "network": world.network, "districting": world.districting_method,
                "partition_id": world.partition_id,
        ...
        self._index_upsert({
            ...
            "n_include": len(spec.include), "n_exclude": len(spec.exclude),
