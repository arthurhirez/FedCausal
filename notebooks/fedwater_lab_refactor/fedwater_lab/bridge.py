...
__all__ = ["nodes_module", "preprocess_clients", "deseasonalize",
           "add_measurement_noise", "AER", "FPLTrainer",
...
def add_measurement_noise(*args, **kw):
    """`sensing.nodes.add_measurement_noise`, verbatim -- the ONE noise

    source. Keyed on the `sensor` column, exactly as the pipeline itself
    keys it (see `train.assert_determinism` for the reproducibility note
    this implies).
    """
    return nodes_module("sensing").add_measurement_noise(*args, **kw)


# --------------------------------------------------------- config / networks
# Plain modules (no kedro import at module scope), so no stub-package dance:
# a direct `import fedwater.<x>` works whether or not kedro is installed.
def load_globals(*args, **kw):
    import fedwater.config as _c
    return _c.load_globals(*args, **kw)


def read_partition(*args, **kw):
    import fedwater.networks.partitions as _p
    return _p.read_partition(*args, **kw)


def partition_meta(*args, **kw):
    import fedwater.networks.partitions as _p
    return _p.partition_meta(*args, **kw)
