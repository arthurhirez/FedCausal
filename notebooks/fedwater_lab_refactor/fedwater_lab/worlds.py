...
Sensors are read from `data/03_primary/sensor_placement.csv` -- the one
contract every world writes, whether its placement source is `dynamic`
(tiered pure/mixed slots) or `manual` (the bundle profile's hand-placed
set). Which one a given world is is a FACT ABOUT THE WORLD
(`world.placement_source`), read from the manifest and cross-checked
against the CSV the first time it is opened; it is not a choice a later
analysis gets to make (see `specs.CellSpec`, which filters WITHIN whatever
a world offers and never assumes a placement source).

`list_worlds`/`pick_world` browse the cached worlds under an experiments
root by their manifest alone -- network, districting method, partition id,
drift target, placement source -- without opening any world's `clone/`, so
picking a world by districting method costs nothing per candidate.
"""
...
__all__ = ["World", "load_world", "list_worlds", "pick_world", "num",
           "deep_merge"]
...
class World:
    ...
    def network(self) -> str | None:
        n = self.meta.get("network")
        return str(n) if n else None

    @property
    def districting_method(self) -> str | None:
        """The partition this world was built on (manual | spectral | ...)."""
        m = self.meta.get("districting")
        return str(m) if m else None

    @property
    def partition_id(self) -> str | None:
        """Content id of that partition -- worlds built on a different cut
        of the same network (even the same method, rebuilt) get a different
        one, so this is the honest key for "same partition", not the method
        name alone."""
        p = self.meta.get("partition_id")
        return str(p) if p else None

    @cached_property
    def districts(self) -> dict[str, list[str]]:
        """{district: [junction, ...]} of the partition this world was built
        on (the clone's own copy, see `partition_info`)."""
        from . import bridge
        doc = bridge.read_partition(self.clone, self.network,
                                    self.districting_method)["districts"]
        return {d: [str(n) for n in ns] for d, ns in doc["districts"].items()}

    @cached_property
    def district_demand(self) -> pd.DataFrame:
        """steps x districts: total demand of each district's junctions
        (`demand_series.parquet`, summed per partition district), plus
        `month`. The demand every sensor ultimately responds to."""
        f = self.clone / "data" / "02_intermediate" / "demand_series.parquet"
        dem = pd.read_parquet(f)
        out = pd.DataFrame({"month": dem["month"].to_numpy()})
        for d, nodes in self.districts.items():
            cols = [n for n in nodes if n in dem.columns]
            out[d] = dem[cols].sum(axis=1).to_numpy()
        return out

    def partition_info(self) -> dict:
        """Sizes, k, boundary links -- `partition.yml` for this world's cut.

        Reads the clone's own copy (`data/01_raw/<network>/partitions/
        <method>/partition.yml`), so it reflects exactly what this world was
        built on even if the project's partition has since been rebuilt.
        """
        from . import bridge
        return bridge.read_partition(self.clone, self.network,
                                     self.districting_method)["manifest"]

    # ----------------------------------------------------------- placements
    @cached_property
    def sensor_placement(self) -> pd.DataFrame:
        """`data/03_primary/sensor_placement.csv` -- THE sensor contract.

        One row per selected sensor, whatever the placement source: `dynamic`
        rows carry the tier/purity/mixture columns that are this sensor's
        reason for being there; `manual` rows carry only identity (see
        `World.placement_source`). `data.sensor_table` is where a `CellSpec`
        filters this.
        """
        f = self.clone / "data" / "03_primary" / "sensor_placement.csv"
        if not f.exists():
            raise FileNotFoundError(
                f"{self.sim_hash}: no {f.name} in this world's clone. Worlds "
                "built before the dynamic-sensors refactor (or built with an "
                "older pipeline) do not have one; this package can only "
                "analyse worlds the current pipeline produced.")
        df = pd.read_csv(f, dtype={"element": str})
        observed = ("manual" if (df["slot_class"] == "manual").all()
                   else "dynamic")
        recorded = self.meta.get("placement_source")
        if recorded is not None and str(recorded) != observed:
            raise AssertionError(
                f"{self.sim_hash}: manifest says placement_source="
                f"{recorded!r} but sensor_placement.csv's slot_class column "
                f"says {observed!r} -- the file does not match the world it "
                "claims to belong to.")
        return df

    @property
    def placement_source(self) -> str:
        """`"dynamic"` (tiered pure/mixed slots) or `"manual"` (the bundle
        profile's hand-placed set) -- a fact about the WORLD, read from the
        manifest and cross-checked against `sensor_placement` the first time
        that is opened. Not a `CellSpec` field: a spec filters within
        whatever a world offers, it never changes what the world IS."""
        recorded = self.meta.get("placement_source")
        return str(recorded) if recorded is not None \
            else ("manual" if (self.sensor_placement["slot_class"] == "manual"
                              ).all() else "dynamic")

    @property
    def client_names(self) -> list[str]:
        return sorted(self.sensor_placement["district"].unique())
    ...
    def wn(self):
        """The wntr network (`wn_variant.pkl`). Imported lazily -- nothing in
        this package's default path needs it."""
        import pickle
    ...
    def summary(self) -> pd.Series:
        ph = self.phases()
        sp = self.sensor_placement
        return pd.Series({
            "sim_hash": self.sim_hash, "tag": self.tag,
            "network": self.network, "districting": self.districting_method,
            ...
            "n_sensors": int(len(sp)),
            "n_pure": int((sp.get("slot_class") == "pure").sum()),
            "n_mixed": int((sp.get("slot_class") == "mixed").sum()),
...
# --------------------------------------------------------------------------
# browsing cached worlds
# --------------------------------------------------------------------------
# The manifest columns worth a glance when picking a world -- everything
# else needs `load_world` (and its clone) to answer.
_LIST_COLS = ("sim_hash", "network", "districting", "partition_id",
             "placement_source", "drift_district", "consumption_map", "beta",
             "n_months", "days_per_month", "seasonality_scale", "variant",
             "sim_seed")


def list_worlds(root: str | pathlib.Path, status: str | None = "ok"
                ) -> pd.DataFrame:
    """One row per cached world under `root` (an experiments root, i.e. the
    directory holding `worlds/`), read from its `manifest.json` ALONE -- no
    `clone/` is opened, so this is cheap even over hundreds of worlds.

    Columns mirror the world-identity `flat` dict the pipeline records
    (`network`, `districting`, `partition_id`, `placement_source`,
    `drift_district`, ...), so browsing by districting method or placement
    source is a plain DataFrame filter:

        w = list_worlds(ROOT / "data" / "09_experiments")
        w[(w.districting == "spectral") & (w.placement_source == "dynamic")]

    `status="ok"` (the default) drops failed builds; pass `None` to see
    everything, including failures (their identity columns are `None`).
    """
    root = pathlib.Path(root).expanduser().resolve()
    wdir = root / "worlds" if (root / "worlds").is_dir() else root
    rows = []
    for mf in sorted(wdir.glob("*/manifest.json")):
        try:
            ...
            continue
        flat = m.get("world") or {}
        rows.append({"path": str(mf.parent), "status": m.get("status", "ok"),
                     "seconds": m.get("seconds"),
                     "created_utc": m.get("created_utc"),
                     **{c: flat.get(c) for c in _LIST_COLS
                        if c != "sim_hash"},
                     "sim_hash": m.get("sim_hash", mf.parent.name)})
    df = pd.DataFrame(rows)
    if status is not None and len(df):
        df = df[df["status"] == status].reset_index(drop=True)
    cols = list(_LIST_COLS) + ["status", "seconds", "created_utc", "path"]
    return df.reindex(columns=cols)      # NaN-fills any column with no rows


def pick_world(root: str | pathlib.Path, **filters) -> pathlib.Path:
    """The one cached world matching every `column=value` filter.

    Raises with the candidate table shown when zero or more than one world
    matches -- `list_worlds` first if you want to browse rather than assert.
    """
    df = list_worlds(root)
    for col, val in filters.items():
        if col not in df.columns:
            raise KeyError(f"pick_world: no column {col!r} "
                           f"(have {list(df.columns)})")
        df = df[df[col] == val]
    if len(df) == 1:
        return pathlib.Path(df.iloc[0]["path"])
    if not len(df):
        raise LookupError(f"no cached world matches {filters}")
    raise LookupError(
        f"{len(df)} worlds match {filters}, pick_world needs exactly one:\n"
